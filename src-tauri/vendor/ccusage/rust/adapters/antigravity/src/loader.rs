...
use ccusage_core::{
    LoadedEntry, PricingMap, Result, debug_log, parse_tz,
    progress::{UsageLoadAgent, track_usage_load},
};
use ccusage_cli::{CostMode, SharedArgs};

pub fn load_entries(shared: &SharedArgs, pricing: &PricingMap) -> Result<Vec<LoadedEntry>> {
    track_usage_load(
        UsageLoadAgent("Antigravity"),
        shared.json,
...
fn load_entries_from_database(
    ...
    // Downstream (Coding Agent Monitor) 0002 patch: the immutable fast path
    // is re-verified after the read; a writer that appeared mid-read discards
    // the immutable result and retries via a plain read-only transaction.
    let loaded = ccusage_adapter_common::load_source_db_stable(db_path, |connection| {
        load_entries_from_connection(connection, db_path, tz, mode, pricing, shared)
    });
    match loaded {
        ...
        Ok((entries, ccusage_adapter_common::SourceStability::ChangedDuringRead)) => {
            debug_log(
                shared,
                "Antigravity source changed during immutable read; recovered via read-only retry"
                    .to_string(),
            );
...
mod tests {
    ...
    fn falls_back_to_trajectory_created_at_then_file_mtime() {
        ...
        assert_eq!(
            ...
            ccusage_core::TimestampMs::from_millis(1_767_312_000_000)
        );
        ...
        assert_eq!(
            ...
            ccusage_core::TimestampMs::from_millis(mtime_ms)
        );
    }
    ...
    fn reads_real_antigravity_data_when_present() {
        ...
        assert!(entries.iter().all(|entry| {
            ccusage_core::total_usage_tokens(entry.data.message.usage) + entry.extra_total_tokens
                > 0
        }));
    }
}
